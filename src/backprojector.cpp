...
//#define DEBUG_BACKP
    ...
    std::cerr << " padding_factor= " << padding_factor << std::endl;
...
void BackProjector::backprojectverbose(const MultidimArray<Complex > &f2d,
                                const Matrix2D<double> &A, bool inv,
                                const MultidimArray<double> *Mweight)
{
    double fx, fy, fz, mfx, mfy, mfz, xp, yp, zp;
        ...
        for (int x=first_x; x <= r_max; x++)
        {
            ...
            if (my_weight > 0.)
            {
                ...
                if (interpolator == TRILINEAR || r2 < min_r2_nn)
                {
                    ...
                    std::cerr << "i: " << count << " X0: " << x0 << " X1: " << x1 << " Y0: " << y0 << " Y1: " << y1 << " Z0: " << z0 << " Z1: " << z1 << " f2d: " << my_val.real << " " << my_val.imag << " fw: " << my_weight << std::endl;
